...
export default function Card(props: {
  ...
  const { size = 'default', subtitle, title, className, children } = props

  return (
    <div className={clsxMerge(cardClass({ size: size }), className)}>
      {title ? (
        <div className="mb-4">
          <h3 className="text-lg-semibold text-strong">{title}</h3>

          {subtitle ? (
            <p className="mt-1 text-sm-regular text-subtle">{subtitle}</p>
          ) : null}
        </div>
      ) : null}
