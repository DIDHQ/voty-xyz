...
export function InfoCard(
  ...
  const { title, desc, thumbnail, className, badge, children, ...restProps } =
    props
  ...
  return (
    <Link
      ...
      {...restProps}
    >
      <div className="flex w-full gap-4 p-4 md:p-5">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            {badge}

            <h3 className="truncate text-lg-semibold text-strong">{title}</h3>
          </div>

          <p className="mt-2 line-clamp-3 text-sm-regular text-moderate">
            {desc}
          </p>
        </div>
      ...
      <div className="flex gap-4 border-t border-base bg-base px-4 pb-4 pt-3 md:px-5">
        {children}
      </div>
    </Link>
  )
...
export function InfoItem(props: {
  ...
  const { label, value, hightlight = false, phaseColor, className } = props

  return (
    <div className={clsxMerge('flex-1 min-w-0', className)}>
