          ...
          <RadioGroup.Option
            ...
            className={({ checked }) =>
              clsxMerge(
                ...
                checked ? 'border-primary-500 hover:border-primary-500' : '',
              )
            }
          >
            {({ checked, disabled }) => (
              <>
                <span
                  className={clsxMerge(
                    ...
                    disabled
                      ? 'cursor-not-allowed'
                      : 'cursor-pointer group-hover:border-strong',
                    checked
                ...
                <div className="flex-1">
                  <RadioGroup.Label className={clsx('text-sm text-strong')}>
                    {option.name}
                  </RadioGroup.Label>

                  <RadioGroup.Description
                    className={clsx('text-xs-regular text-subtle')}
                  >
                    {option.description}
                  </RadioGroup.Description>
                </div>
