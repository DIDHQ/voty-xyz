...
const button = tv({
  ...
  variants: {
    color: {
      default: 'border-[#2D2E2E] bg-[#2f2f2f] text-white enabled:hover:bg-[#1E2022]',
      primary: 'border-transparent bg-primary-500 text-white enabled:hover:bg-primary-600'
    },
    size: {
  ...
  compoundVariants: [{
    ...
    color: 'primary',
    outline: true,
    class: 'border-2 border-primary-500 text-primary-500 enabled:hover:bg-primary-500/5'
  }, {
    size: 'default',
    square: true,
    class: 'w-9'
...
export default function Button(
  ...
  const { base: buttonClass, loader: loaderClass } = button({ color: color, size: size, outline: outline, square: square })
  ...
  return (
    <button
      ...
      {loading ? (
        ...
        <Icon
          className={clsx(children ? '-ml-1' : undefined, 'h-5 w-5')}/>
      ) : null}
